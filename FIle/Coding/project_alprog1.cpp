...
void OutputData(parkir &p, int sumData){
	...
	for(int i = 0; i < sumData; i++){
		cout << "\t" + string(90,'=') << endl;
		cout << "\t" <<"|" << setw(5) << i + 1 << setw(22) << p.namaKendaraan[i] << setw(28) << p.plat[i] << setw(22) << p.jam[i] << setw(12) << "|" << endl;
	}
	cout << "\t" + string(90,'=') << endl;
}
...
void Merge(string type){
	...
	do{
		...
		string file2 = ReadFile();
			...
			parkir p;
			ifstream myFile;
			myFile.open(file1); // open file1
			while(getline(myFile,p.plat[i])){ // insert to myFile
			...
			ofstream inFile;
			inFile.open(namaFile, ios::trunc); //insert to new File
			for(int j=0; j<i; j++){
